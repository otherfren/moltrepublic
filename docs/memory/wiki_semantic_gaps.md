...
- Priority order: §1.3 (index the header text and aliases) and §2.1 (a
  property-value filter) are each about a day and both raise recall
  immediately; §1.1 is the real feature. §2.2 and §2.4 follow.
- §6 below - typed links in prose - is under discussion and changes
  where relations live. Decide it BEFORE §1.1 is built: the structured
  write path's shape depends on it (an `add_relation` op writes a header
  key or a sentence, not both).
...
## 6. Under discussion: typed links in the PROSE, not the header

Raised by the user 2026-09-05: relations in the front matter are the
wrong place; a link in a sentence should be able to carry its relation
keyword, and the preview should show that keyword as a tooltip over the
link. That is Semantic MediaWiki's `[[works_at::Acme]]`, and it is
already named as deferred in `knowledge_base_scale.md` §6.

Nothing fundamental speaks against it. What has to be decided with it:

1. **Relations are a SET, so both sources can coexist without a conflict
   rule.** A header `works_at: "[[Acme]]"` and a sentence
   `[[works_at::Acme]]` produce the same edge; two sources can only ADD
   edges, never contradict each other the way two values of one field
   would. So "prose becomes the primary form" needs no migration and no
   precedence rule - the header form simply stops being the one a human
   is pushed towards. Live republics keep verifying.
2. **The header does not disappear.** `type`, `tags`, `aliases` and plain
   scalars (`born: 1975`) have no sensible inline form. The split then
   reads cleanly: the header says what a page IS, the prose says what it
   RELATES to.
3. **Qualified relations lose their home.** `works_at: { to, since, role }`
   is expressible only in the header (SMW uses subobjects for this, which
   we do not have). Either keep the header form for qualified relations,
   or accept that a qualifier becomes prose a machine does not read.
4. **Parsing is one place.** `wiki_index/graph.rs::body_links` already
...
5. **The tooltip needs the predicate on the span.** `WikiSpan` (theme.slint)
...
6. **It is not standard markdown.** `[[pred::Name]]` renders as literal
...
7. **It makes §1.2's decision cost MORE visible, not less.** More
   predicates will exist, all of them uninterpreted strings. `wiki_props`
   must then inventory the inline predicates too, or the one call that
   answers "what relations does this republic use" would miss the form
   the republic actually writes.
