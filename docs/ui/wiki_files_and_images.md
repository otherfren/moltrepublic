...
Status: **CONCEPT for discussion (2026-09-06).** Nothing of this is built.
Commissioned while the wiki pane performance wave
(`wiki_pane_performance.md` §4 steps 2-6) was in flight; this plan builds
ON TOP of that wave (its preview cache and block equality are touched
here, §3.4) and must not start before it has merged.
...
digits, resolved like a git short hash: exactly one persisted-or-shared
file with that prefix, else "ambiguous" (shown, never guessed). Plain
...
match: Option<UploadView>, ambiguous: bool, local: LocalCopy }` with
`LocalCopy = None | Own { path } | Downloaded { path } | Mirrored | Partial
{ held, of }`. MCP tool `resolve_upload` (Read scope) - an agent that meets
...
6 ready · 7 failed · 8 expired) and `image` (`slint::Image`, empty until
...
`image_from_bytes` limits + a downscale to at most 1600 px on the long
edge), and hands an RGBA buffer to the UI thread, where the `slint::Image`
...
- `remote` (known, not here): a bordered card in the text column: kind
  ...
  A non-persistent share adds `läuft ab in 13 Tagen` in the warn tone.
...
- `ready`: the picture, width = the text column, height by aspect, the
  alt text as caption in the dim tone; a click jumps to Shared Files
  filtered to the checksum (the one navigation rule for every file
  reference, §3.5); the tooltip carries name · size · sharer.
- `unknown` / `ambiguous` / `failed` / `expired`: the card with the one
  word and the reference prefix, in the bad tone; a `failed` card offers
  `Erneut laden`.
A file link run (`FileRun`, sibling of `LinkRun`): `📎 name` with
`(size)` in the dim tone; state colours as above; a click jumps.
...
needle is the FULL checksum (unique), `SelectView { Files, "uploads" }`
(or `"persistent"` when the match is persistent - the row lives in that
table), surfaces refreshed. The filter field shows the needle, so the
...
modal listing the republic's uploads (persistent first, then temporary
with their expiry; a search field over name/sharer; kind glyphs), one
click inserts `![name](upload:<12-hex>)` for an Image kind or
...
- `wiki_health` gains `files: { dangling, expiring, ambiguous }` so an
  agent's hygiene pass finds rotting references.
...
- Bytes render only after the sha256 matched the reference (§3.3); the
  ...
  picture.
- Decoding under `image::Limits` (8192x8192), a 32 MiB read cap, a 1600 px
...
3. **Pane.** Parser kinds, additive model fields, bridge resolution +
   ...
   refresh; headless GUI tests: every state renders its word, the click
   sets the needle and the view, an arriving image patches the row in
   place (a 2x2 PNG fixture), the perf probe's frame after a resolve stays
   partial. i18n DE/EN for every new string.
...
## 6. Open questions (decide before stage 1)

- **Q1 Grammar.** `upload:<hex>` with a 12-hex minimum prefix - agreed?
  (Alternative: full hashes only, uglier in prose but never ambiguous.)
- **Q2 Temporary shares.** Allowed in a page with the expiry warning
  (proposed), or persistent-only in the picker AND the renderer?
- **Q3 A click on a READY image**: jump to Shared Files like every
  reference (proposed, one rule), or open the picture large in place?
- **Q4 Auto-fetch.** A referenced image on a remote-only share is
  downloaded only on the card's verb (proposed - traffic and privacy stay
  the member's call; mirrored persistent files arrive by themselves). Or
  an `[ui] auto_fetch_images` setting?
- **Q5 The registry's home**: workspace prefs (proposed, beside
  `read_cursors`) or `transport.state`?
