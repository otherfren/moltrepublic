...
pub fn tools() -> Vec<ToolDef> {
    vec![
        ...
        ToolDef {
            ...
            scope: Scope::Seat,
            description: "Download a shared file into the exchange folder. When this seat already MIRRORS the whole file (read_uploads `local: mirrored`) the bytes are assembled locally and no network is involved - otherwise they are fetched peer-to-peer from the sharer's device over a dedicated encrypted queue (the sharer must be online). Either way it verifies size + sha256 against the share, and writes the file into the node's download directory (the EXCHANGE FOLDER) - as `dest`, a bare file name, or under the share's own name when omitted. Never an arbitrary path: peer-chosen bytes landing anywhere on the operator's machine would be a persistence primitive. Async kickoff - poll read_uploads for the download's phase/percent/path/error. Addressed by the share message's stable id (32-char lowercase hex, from read_state). Fails honestly once the sharer deleted the file or stays offline.",
            schema: || json!({
                "type": "object",
                "properties": {
