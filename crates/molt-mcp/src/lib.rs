...
pub fn tools() -> Vec<ToolDef> {
    vec![
        ...
        ToolDef {
            ...
            command: "approve",
            scope: Scope::Seat,
            description: "Contribute THIS node's approval toward a pending proposal. On a chain-governed republic it is a real signature gossiped to the mesh (the block seals once m distinct members signed); elsewhere the node records at most its own single approval - it can never approve on behalf of other members. The reply is the record after the vote (state, approvals/threshold, `head`, `channel`); approving an already-applied proposal answers the same shape - a late approval is not an error. `approvals` counts the signatures THIS node holds; the block may already be sealed on a peer - read_chain shows the head. `state: \"sealing\"` with `held_for_secs` means the threshold is reached here and the seal waits out one propagation round.",
            schema: || json!({
                "type": "object",
        ...
        ToolDef {
            ...
            command: "wiki_edit",
            scope: Scope::Seat,
            description: "Write the wiki. `edits` apply IN ORDER to a working copy of the current base; the engine diffs the result and puts THAT patch to the members as a normal changeset vote - propose, not save, and the reply is the proposal id. Ops (fields in the schema): content, replace, set_props, add_relation, rename, delete - content also CREATES a document when the path is free. An edit that cannot land refuses the whole call with the reason and proposes nothing: a path that does not exist, an `old` that is absent or occurs more than once, a header the parser would not read back, a relation target that does not resolve uniquely, a rename onto an existing path. A path another OPEN proposal also touches is no refusal either: that patch goes to the vote and reads `superseded` once the other one seals and moves the base. `replace` matches a SUBSTRING, not a line: `## Quellen` also sits inside `### Quellen (Sitz B)`, so anchor on `\\n## Quellen\\n`; a repeated one-line `old` is refused with the line numbers it hit. A relation reads better in the sentence that asserts it - write `[[predicate::Name]]` inside a content or replace edit; add_relation is for when there is no such sentence. An edge asserted BOTH in the header and in the prose survives set_props removing the header key - the inline link still asserts it (wiki_links marks each edge `header: true/false`). Check a target with wiki_resolve, and wiki_props for the relation names this republic already uses. The proposer's own signature is the first of m. The vote card's `summary` counts `+` `-` `→` in FILES added, deleted and renamed, `~` in changed LINES. `dry_run: true` proposes nothing and answers `summary`, `warnings` and the `paths` it would touch; `with_patch: true` adds the patch itself. A result whose header the parser reads differently than written (an unquoted `[[link]]` value is a nested list) is REFUSED with the key named; `allow_warnings: true` proposes it anyway. `supersedes: <id>` withdraws that own, still-open proposal in the same step - the way to correct one already on the table. The header dialect is the YAML 1.2 core schema in a flat subset: `no`, `yes`, `on` stay strings. The reply names the proposal's `channel`, where review remarks belong.",
            schema: || json!({
                "type": "object",
                "properties": {
                    "edits": {
