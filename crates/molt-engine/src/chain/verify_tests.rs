...
/// Shared Files came after checkpoint-v7: a cut carries its group only
/// once something was applied under it, so every earlier checkpoint keeps
/// its bytes AND its JSON shape (an older build still parses the blob).
#[test]
fn a_later_surface_joins_the_checkpoint_only_with_state() {
    let mut b = Builder::new(&["petra", "walter", "dora"], 2);
    b.commit_org(1, "set_name", "X", &["petra", "walter"]);
    let before = checkpoint_state(&b.blocks, 1).expect("state@1");
    assert_eq!(
        before.applied.iter().map(|(s, _)| *s).collect::<Vec<_>>(),
        Surface::CHECKPOINT_V7_SURFACES.to_vec(),
        "no group for a surface that holds nothing yet"
    );
    let json = serde_json::to_string(&before).expect("blob serializes");
    assert!(!json.contains("\"files\""), "the blob shape is the v7 one: {json}");
    ...
    let block = b.seal(
        ...
        ChainChange::Applied {
            ...
            payload: json!({ "op": "share", "value": "x" }),
        },
        &["petra", "walter"],
    );
    b.push(block);
    let after = checkpoint_state(&b.blocks, 2).expect("state@2");
    let (_, files) = after
        .applied
        .iter()
        .find(|(s, _)| *s == Surface::Files)
        .expect("the group appears with its first entry");
    assert_eq!(files.len(), 1);
    assert!(
        molt_core::checkpoint_canonical_bytes(&after).starts_with(b"molt-chain-checkpoint-v8\0"),
        "state under the later surface moves the tag"
    );
    // and the incremental walk every verifier runs agrees with the fold
    let cut = b.seal(
        3,
        ChainChange::Checkpoint {
            upto: 2,
            state_hash: checkpoint_state_hash(&after),
        },
        &["petra", "walter"],
    );
    b.push(cut);
    verify_chain(&b.blocks).expect("the walk recomputes the v8 cut");
}

/// A served blob carrying a phantom EMPTY group for a later surface hashes
/// exactly like the honest one (the group is invisible to the preimage),
/// so the signed hash cannot catch it - the shape check must.
#[test]
fn a_blob_with_a_phantom_later_group_is_refused_despite_the_matching_hash() {
    let mut b = Builder::new(&["petra", "walter", "dora"], 2);
    b.commit_org(1, "set_name", "X", &["petra", "walter"]);
    b.commit_applied(2, &["petra", "walter"]);
    ...
    let mut phantom = honest.clone();
    phantom.applied.insert(0, (Surface::Files, Vec::new()));
    assert_eq!(
        checkpoint_state_hash(&phantom),
        checkpoint_state_hash(&honest),
        "the phantom group is invisible to the hash - that is the point"
    );
    let err = verify_suffix_chain(&phantom, &suffix, &b.republic_id)
        .expect_err("the shape check refuses it");
    assert!(err.contains("v7 surface set"), "{err}");
}
