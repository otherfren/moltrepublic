...
struct Queue {
    ...
    /// (`docs_archive/transport/mesh/mesh_selfheal.md` §6): when set, `SUB` and `SEND` still
...
impl LoopbackHub {
    ...
    /// `docs_archive/transport/mesh/mesh_selfheal.md` §6): from now on `SUB`/`SEND` still return
