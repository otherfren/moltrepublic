...
use crate::labels::{
    backup_when_label, charter_columns, file_size_label, chain_diverged_copy, genesis_undelivered_copy, vote_refused_copy, from_screen,
...
pub(crate) fn apply_session(
    ...
    if ui.get_recover_notice_seen() != sv.notice.as_str() {
        ...
        } else if let Some(rest) = sv.notice.strip_prefix("vote-refused:") {
            // A6: the vote this seat cast was dropped by a peer that is ahead
            ui.invoke_show_toast_error(vote_refused_copy(lang, rest).into());
