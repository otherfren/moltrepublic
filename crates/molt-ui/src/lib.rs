...
fn localize_error(lang: i32, e: &molt_core::MoltError) -> String {
    ...
    match e {
        ...
        E::WikiExport(t) => format!(
            "Wiki-Export: {}",
            match *t {
                "a target directory is required" => "ein Zielordner fehlt",
                "an export is already running" => "es läuft bereits ein Export",
                "the wiki is empty" => "das Wiki ist leer",
                "proof needs chain governance" => "das Prüfpaket braucht Chain-Governance",
                "proof needs the genesis block" => "das Prüfpaket braucht den Genesis-Block",
                other => other,
            }
        ),
